...
import Node3D
from Manifest import NodegraphAPI, GeoAPI, PyScenegraphAttr, FnAttribute
...
class MergeNode3D(Node3D.Node3D):
    ...
    def __init__(self):
        ...
        try:
            ...
            exception = None
            None
            log.exception('Error initializing %s node: %s' % (self.__class__.__name__, str(exception)))
    ...
    def _getOpChain(self, interface):
        ...
        showAdvanced = self.getParameter('showAdvancedOptions').getValue(frameTime) == 'Yes'
        if showAdvanced:
            continue
            names = [x.getValue(frameTime) for x in
                     self.getParameter('advanced.preferredInputAttributes.name').getChildren()]
            continue
            indices = [int(x.getValue(frameTime)) for x in
                       self.getParameter('advanced.preferredInputAttributes.index').getChildren()]
            maxLength = min(len(names), len(indices))
            names = names[:maxLength]
            indices = indices[:maxLength]
            if len(names):
                inputBehavior = interface.NO_OP
                argsGb.set('advanced.preferredInputAttributes.attrNames', FnAttribute.StringAttribute(names))
                argsGb.set('advanced.preferredInputAttributes.inputIndices', FnAttribute.IntAttribute(indices))
            continue
            for v in [x.getValue(frameTime) for x in
                      self.getParameter('advanced.preserveInheritedAttributes').getChildren()]:
                if v:
                    continue
                    preserveAttrs = [][v]
                    if preserveAttrs:
                        argsGb.set('advanced.preserveInheritedAttributes', FnAttribute.StringAttribute(preserveAttrs))
                    if self.getParameter('advanced.sumBounds').getValue(frameTime) == 'Yes':
                        argsGb.set('advanced.sumBounds', FnAttribute.IntAttribute(1))
                    if self.getParameter('advanced.preserveWorldSpaceXform').getValue(frameTime) == 'Yes':
                        inputBehavior = interface.NO_OP
                        argsGb.set('advanced.preserveWorldSpaceXform', FnAttribute.IntAttribute(1))
                    preferRightmostInputWithAttributeName = self.getParameter(
                        'advanced.preferRightmostInputWithAttribute').getValue(frameTime)
                    if preferRightmostInputWithAttributeName:
                        argsGb.set('advanced.preferRightmostInputWithAttribute',
                                   FnAttribute.StringAttribute(preferRightmostInputWithAttributeName))
                    continue
                    mergeAttrs = None([], [childParam.getValue(frameTime) for childParam in
                                           self.getParameter('advanced.mergeGroupAttributes').getChildren()])
                    if mergeAttrs:
                        argsGb.set('advanced.mergeGroupAttributes', FnAttribute.StringAttribute(mergeAttrs))
                ...
                if numValidInputs > 1 or opArgs.getNumberOfChildren() > 0:
                    interface.appendOp('Merge', opArgs)
                return None
...
NodegraphAPI.RegisterPythonNodeType('Merge', MergeNode3D)
NodegraphAPI.AddNodeFlavor('Merge', '3d')
